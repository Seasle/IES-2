import { regions, cities, rules, IFilteredRules } from './store.js';
...
const update = () => {
    ...
    if (controls.has('input')) {
        const sentence = (controls.get('input') as HTMLInputElement).value;
        const words = sentence.split(/\s+/g);
        const lowerWords = words.map((word: string) => word.toLowerCase());

        const filteredRules: IFilteredRules[] = rules
...
const bind = () => {
    ...
    if (controls.has('input')) {
        controls.get('input').addEventListener('input', (event: InputEvent) => {
            if (containers.has('spinner')) {
    ...
    const updateElements = (container: HTMLElement, control: HTMLElement) => {
